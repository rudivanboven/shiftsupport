...
export default function WorkersPage(){return <><Header/><main className={styles.page}>
  <div className="container"><AccountStrip audience="worker" returnTo="/workers"/></div>
  <section className={styles.hero}><div className="container"><div className={styles.heroGrid}><div className={styles.heroCopy}><Eyebrow>FOR LOCAL WORKERS</Eyebrow><h1>Find <span>Flexible Work</span><br/>That Fits Your Schedule.</h1><p className={styles.lead}>Pick up shifts that fit your life.</p><p>Find short, flexible shifts with local retailers and choose opportunities that work around your school, schedule, and availability.</p><div className={styles.facts}><span>2–4 hour shifts</span><span>Local work</span><span><strong>Flexible</strong> schedule</span></div><div className={styles.actions}><div><small>New to ShiftSupport?</small><a className={styles.solidButton} href="/worker/signup">Create Worker Account →</a></div><div><small>Already have an account?</small><a className={styles.outlineButton} href="/worker/login">Log In →</a></div></div></div><div className={styles.heroVisual}><img className={styles.mainImage} src="/images/ededd.png" alt="Young local worker ready for a retail shift"/><img className={styles.smallImage} src="/images/ewe.png" alt="Worker using a phone to check local opportunities"/><Label className={styles.labelOne}>LOCAL WORK</Label><Label className={styles.labelTwo}>FLEXIBLE HOURS</Label><Label className={styles.labelThree}>READY TO EARN</Label></div></div></div></section>
  ...
  <WorkerResources/>
  <section className={styles.nearSection}><div className={`container ${styles.nearGrid}`}><div className={styles.nearCopy}><Eyebrow>LOCAL BY DESIGN</Eyebrow><h2>Find <span>Shifts</span> Near You.</h2><h3>Sign up, log in, and see what’s available in your neighborhood.</h3><p>Short shifts can appear in real time with local retailers and businesses.</p><p>Review the opportunity, location, schedule, and duties, then choose the shifts that work for you.</p><Checks items={["Local opportunities","Short flexible shifts","Clear shift details","Work around your availability"]}/><a className={styles.solidButton} href="/worker/signup">Create Worker Account →</a></div><div className={styles.nearVisual}><img src="/images/rrr.jpg" alt="Neighborhood retail environment offering local shifts"/><Label>OPPORTUNITIES NEARBY</Label></div></div></section>
  <section className={styles.accountSection}><div className={`container ${styles.accountBanner}`}><img src="/images/rr.jpg" alt="Worker helping inside a local shop"/><div className={styles.overlay}/><div className={styles.accountCopy}><p>WORKER ACCOUNT</p><h2>Ready to Start Working?</h2><p>Create your account, verify your identity, complete the required documents, and start exploring local opportunities that fit your schedule.</p><a className={styles.lightButton} href="/worker/signup">Create Worker Account →</a></div></div></section>
  <section className={styles.finalCta}><div className={`container ${styles.ctaInner}`}><div><Eyebrow>YOUR SCHEDULE, YOUR CHOICE</Eyebrow><h2>Ready to Find Work That Fits Your Life?</h2><p>Create your worker account and start exploring flexible local opportunities.</p></div><a className={styles.solidButton} href="/worker/signup">Create Worker Account →</a></div></section>
  <Positioning/>
  </main><Footer/></>}
